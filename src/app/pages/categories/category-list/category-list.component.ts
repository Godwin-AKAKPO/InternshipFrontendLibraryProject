    ...
    <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div *ngFor="let cat of categories" class="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
          <div class="flex items-center gap-2 mb-2"><h3 class="font-semibold text-gray-800">{{ cat.name }}</h3></div>
          <p class="text-sm text-gray-500">{{ cat.description || 'Pas de description' }}</p>
          <p class="text-xs text-gray-400 mt-3 pt-3 border-t border-gray-100">ID: {{ cat.id }}</p>
        </div>
        <div *ngIf="categories.length === 0" class="col-span-3 text-center py-10 text-gray-400">Chargement...</div>
      </div>
    </div>
  `
})
